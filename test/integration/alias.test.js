...
describe("integration: alias", () => {
	...
	test("packages: eik alias <name> <version> <alias>", async () => {
		const eik = join(__dirname, "..", "..", "index.js");
	...
	test("npm: eik alias <name> <version> <alias> --token --server : no eik.json or .eikrc", async () => {
		const eik = join(__dirname, "..", "..", "index.js");
	...
	test("npm: eik alias <name> <version> <alias> : publish details provided by eik.json file", async () => {
		const assets = {
			name: "test-app",
			type: "npm",
			version: "1.0.0",
		...
		await fs.writeFile(join(folder, "eik.json"), JSON.stringify(assets));
		const eik = join(__dirname, "..", "..", "index.js");
		const cmd = `node ${eik} alias scroll-into-view-if-needed 2.2.24 2 --token ${token} --cwd ${folder}`;
	...
	test("map: eik alias <name> <version> <alias> --token --server : no eik.json or .eikrc", async () => {
		const eik = join(__dirname, "..", "..", "index.js");
	...
	test("map: eik alias <name> <version> <alias> : publish details provided by eik.json file", async () => {
		const assets = {
			name: "test-app",
			type: "map",
			version: "1.0.0",
		...
		await fs.writeFile(join(folder, "eik.json"), JSON.stringify(assets));
		const eik = join(__dirname, "..", "..", "index.js");
		const cmd = `node ${eik} alias test-map 1.0.0 1 --token ${token} --cwd ${folder}`;
